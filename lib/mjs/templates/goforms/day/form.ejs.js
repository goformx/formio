export default function(ctx) {
var __t, __p = '', __j = Array.prototype.join;
function print() { __p += __j.call(arguments, '') }
__p += '<% const cssClasses = require(\'../cssClasses\'); %>\n<div class="<%= cssClasses[\'form-day\'] %>" role="group"\n  ';
 if (ctx.component.hideLabel) { ;
__p += '\n  aria-label="' +
((__t = (ctx.component.label)) == null ? '' : __t) +
'"\n  ';
 } else { ;
__p += '\n  aria-labelledby="l-' +
((__t = (ctx.instance.id)) == null ? '' : __t) +
'-' +
((__t = (ctx.component.key)) == null ? '' : __t) +
'"\n  ';
 } ;
__p += '\n  ';
 if (ctx.component.description) { ;
__p += '\n  aria-describedby="d-' +
((__t = (ctx.instance.id)) == null ? '' : __t) +
'-' +
((__t = (ctx.component.key)) == null ? '' : __t) +
'"\n  ';
 } ;
__p += '\n  >\n  ';
 if (ctx.dayFirst && ctx.showDay) { ;
__p += '\n  <div class="<%= cssClasses[\'form-day-field\'] %>">\n    ';
 if (!ctx.component.hideInputLabels) { ;
__p += '\n    <label for="' +
((__t = (ctx.component.key)) == null ? '' : __t) +
'-day" class="<%= cssClasses[\'form-day-label\'] %> ';
 if(ctx.component.fields.day.required) { ;
__p += '<%= cssClasses[\'form-day-required\'] %>';
 } ;
__p += '">' +
((__t = (ctx.t('day'))) == null ? '' : __t) +
'</label>\n    ';
 } ;
__p += '\n    <div class="<%= cssClasses[\'form-day-input\'] %>">' +
((__t = (ctx.day)) == null ? '' : __t) +
'</div>\n  </div>\n  ';
 } ;
__p += '\n  ';
 if (ctx.showMonth) { ;
__p += '\n  <div class="<%= cssClasses[\'form-day-field\'] %>">\n    ';
 if (!ctx.component.hideInputLabels) { ;
__p += '\n    <label for="' +
((__t = (ctx.component.key)) == null ? '' : __t) +
'-month" class="<%= cssClasses[\'form-day-label\'] %> ';
 if(ctx.component.fields.month.required) { ;
__p += '<%= cssClasses[\'form-day-required\'] %>';
 } ;
__p += '">' +
((__t = (ctx.t('month'))) == null ? '' : __t) +
'</label>\n    ';
 } ;
__p += '\n    <div class="<%= cssClasses[\'form-day-input\'] %>">' +
((__t = (ctx.month)) == null ? '' : __t) +
'</div>\n  </div>\n  ';
 } ;
__p += '\n  ';
 if (!ctx.dayFirst && ctx.showDay) { ;
__p += '\n  <div class="<%= cssClasses[\'form-day-field\'] %>">\n    ';
 if (!ctx.component.hideInputLabels) { ;
__p += '\n    <label for="' +
((__t = (ctx.component.key)) == null ? '' : __t) +
'-day" class="<%= cssClasses[\'form-day-label\'] %> ';
 if(ctx.component.fields.day.required) { ;
__p += '<%= cssClasses[\'form-day-required\'] %>';
 } ;
__p += '">' +
((__t = (ctx.t('day'))) == null ? '' : __t) +
'</label>\n    ';
 } ;
__p += '\n    <div class="<%= cssClasses[\'form-day-input\'] %>">' +
((__t = (ctx.day)) == null ? '' : __t) +
'</div>\n  </div>\n  ';
 } ;
__p += '\n  ';
 if (ctx.showYear) { ;
__p += '\n  <div class="<%= cssClasses[\'form-day-field\'] %>">\n    ';
 if (!ctx.component.hideInputLabels) { ;
__p += '\n    <label for="' +
((__t = (ctx.component.key)) == null ? '' : __t) +
'-year" class="<%= cssClasses[\'form-day-label\'] %> ';
 if(ctx.component.fields.year.required) { ;
__p += '<%= cssClasses[\'form-day-required\'] %>';
 } ;
__p += '">' +
((__t = (ctx.t('year'))) == null ? '' : __t) +
'</label>\n    ';
 } ;
__p += '\n    <div class="<%= cssClasses[\'form-day-input\'] %>">' +
((__t = (ctx.year)) == null ? '' : __t) +
'</div>\n  </div>\n  ';
 } ;
__p += '\n</div>\n<input name="ctx.data[day]" type="hidden" class="<%= cssClasses[\'form-day-hidden\'] %>" lang="en" value="" ref="input">\n';
return __p
}